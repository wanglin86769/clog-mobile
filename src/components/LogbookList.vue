<template>
	<div>
		<DataView :value="logbooks" layout="list" :paginator="true" :rows="10"
			...
			<template #header>
				<div style="font-size: large; color: RGB(33, 150, 243); text-align: center;">
					<i style="margin-right: .5em" class="pi pi-book"></i>
					...
					<div style="padding: 1rem;">
						<div class="grid border shadow-2">
							...
							<div class="col-6 border-top" :style="{ 'background-color': logHeaderColor(slotProps.index) }">
								<span v-if="slotProps.data.group">{{ slotProps.data.group.name }}</span>
							</div>
						</div>
					</div>
				</div>
